 ...
 */
namespace OCA\Files_Sharing\Collaboration\Collaborators;
...
class UserPlugin implements ISearchPlugin {
	...
	public function search($search, $limit, $offset, ISearchResult $searchResult): bool {
        $this->logger->debug(" - NEW search($search, $limit, $offset)");
        $this->logger->debug("searchResult: " . print_r($searchResult, true));

		// The core user search has already been done so we need to empty the result set, until
		// we figure out the way to prevent that search.
		// Instantiate a SearchResult object through the SimpleContainer.
		$container = new SimpleContainer();
		$searchResult = $container->resolve(SearchResult::class);
		...
		foreach ($users as $uid => $user) {
			...
			$this->logger->debug("uid = $uid, userDisplayName = $userDisplayName, userEmail = $userEmail");
        ...
        $this->logger->debug(" - returning neeewwww ");
